...
def process_batch(batch):
    ...
    test, question, tag, correct, totaltime, a,b,c,d,e,f,g ,mask = batch
    ...
    totaltime = ((totaltime + 1) * mask).to(torch.int64)
    
    a = ((a + 1) * mask).to(torch.int64)
    b = ((b + 1) * mask).to(torch.int64)
    c = ((c + 1) * mask).to(torch.int64)
    d = ((d + 1) * mask).to(torch.int64)
    e = ((e + 1) * mask).to(torch.int64)
    f = ((f + 1) * mask).to(torch.int64)
    
    acc = ((g + 1) * mask).to(torch.int64)
    
    return (test, question, tag, correct, mask, interaction,
             totaltime, a, b, c, d, e, f, acc,
            )
