...
def inference(args, test_data, model):
    ...
    write_path = os.path.join(args.output_dir, "submission"+args.name+".csv")
...
def process_batch(batch):
    ...
    (test, question, tag, correct, totaltime,
     a, b, c, d, e, f, 
     g, h, i, j, 
     mask ) = batch
    ...
    validatetotaltime = ((totaltime + 1)*mask).int()
    
    return (test, question, tag, correct, mask, interaction, totaltime.to(torch.int64)
            , a.to(torch.int64), b.to(torch.int64), c.to(torch.int64), d.to(torch.int64), e.to(torch.int64), f.to(torch.int64), 
            g.to(torch.int64), 
            # h.to(torch.float32), i.to(torch.float32), j.to(torch.float32)
            )
