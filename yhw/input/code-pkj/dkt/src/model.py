...
class LSTMATTN(nn.Module):
    def __init__(self, args):
        ...
        self.embedding_cate = nn.Embedding(
            self.all_dims, self.hidden_dim // 2, padding_idx=0
        )
        ...
        self.mat = torch.nn.MultiheadAttention(
            self.args.hidden_size, num_heads=self.n_heads, dropout=self.drop_out
        )
    ...
    def forward(self, input):
        ...
        batch_size = interaction.size(0)
        field_dims = (
            np.array([0, *np.cumsum(list(self.args.n_embdings.values()))[:-1]]) + 2
        )
        ...
        if self.args.pos:
            ...
            sequence_output, _ = self.mat(
                (out)[-1:, :, :], out + embed, out + embed
            )  # attn_mask=att_mask
...
class LSTMATTN2(nn.Module):
    def __init__(self, args):
        ...
        self.embedding_conti = nn.Sequential(
            nn.Linear(
                len(args.conti_loc),
                self.args.hidden_size // 2 + self.args.hidden_size % 2,
            ),
    ...
    def forward(self, input):
        ...
        if self.args.pos:
            ...
            encoded_layers = self.attn(
                embed, extended_attention_mask, head_mask=head_mask
            )
            sequence_output = encoded_layers[-1]
        else:
...
class Bert2(nn.Module):
    def __init__(self, args):
        ...
        self.args.hidden_size = (
            self.args.hidden_size - self.args.hidden_size % self.args.n_heads
        )
        # embedding combination projection
        num_fe_cols = len(args.cate_feats)
        self.embedding_cate = nn.Sequential(
            nn.Linear(
                self.hidden_dim // 3 * num_fe_cols,
                self.args.hidden_size // 2 + self.args.hidden_size % 2,
            ),
            nn.LayerNorm(self.args.hidden_size // 2 + self.args.hidden_size % 2),
        )
...
class ATTNLSTM(nn.Module):
    def __init__(self, args):
        ...
        self.cate_proj = nn.Sequential(
            nn.Linear(
                self.hidden_dim // 2 * num_cate_cols, self.args.hidden_size // 8 * 7
            ),
    ...
    def forward(self, input):
        ...
        cont_feats = conti["Time"].unsqueeze(
            2
        )  # torch.stack([col for col in conti.values()], 2)
